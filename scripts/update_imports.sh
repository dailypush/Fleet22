#!/bin/bash
# Update imports in refactored scripts
...
echo "📝 Updating scraper scripts..."
cd scrapers

# Update scrape_sail_tags.py to use new utilities
if [ -f "scrape_sail_tags.py" ]; then
    # Create temporary updated version
    cat > scrape_sail_tags.py.new << 'EOF'
import requests
...
if __name__ == "__main__":
    main()
EOF
    mv scrape_sail_tags.py.new scrape_sail_tags.py
    echo "✓ Updated scrape_sail_tags.py"
fi
...
echo ""
echo "Next steps:"
echo "  1. Test a script: python -m scrapers.scrape_sail_tags"
echo "  2. Review changes: git diff"
echo "  3. Test all scripts individually"
echo ""
